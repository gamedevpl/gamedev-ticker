define("Ticker", ["dojo/_base/Deferred", "dojo/string", "dojo/on", "dojo/regexp"], function(Deferred, string, on, regexp) {
		...
		var STORED_MAX_AGE = 1000 * 60 * 60 *24;
		var OPTS_MAX_AGE = 1000 * 60 * 30;
		var EVTS_MAX_AGE = 1000 * 60 * 30;
		var PING_MAX_AGE = 1000 * 60 * 60 * 24 * 31;
		var EVTS_LIMIT = 128;
		...
		var TPL_TITLE = {
			...
			"comment-answer": '<span class="user">${author_name}</span> do <span class="user">${user_name}</span> w <a href="${url}">${title}</a>',
		...
		return {
			init: function(tickerNode) {
				...
				getOpts().then(function(opts) {
							...
							listEvents(filter, offset).then(handler = function(events) {														
								...
								events.some(function(event) {
									...
									try {
										...
										var template = TPL_TITLE[event.group] || TPL_TITLE[opt.group] || TPL_TITLE.DEFAULT;
										if(dojo.isFunction(template))
											event.title_part = template(event);	
										else
											event.title_part = string.substitute(template, event);										
					...
					// ping
					var lastPing = getStored("lastPing", PING_MAX_AGE);
					ping = defer(function() {
						remote({ action: 'ping'}).then(function(result){
							if(!lastPing || lastPing.headTS < result.headTS)
								on.emit(refreshNode, "click", {});
							else
								this.resolve();	
								
							store("lastPing", lastPing = result);
						}.bind(this));
						setTimeout(ping, 60000);
					});
					
					// initial load of events, after ping
					ping().then(function() {					
						loadEvents(activeFilters(), input.value);
					})
				...
				// vacuum storage
				function vacuumStore() {
					(getStored("eventKeys")||[]).map(function(key) {
						getStored(key, STORED_MAX_AGE);
					}.later())
				}
